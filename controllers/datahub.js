...
const queryDatahub = async (sql, onlyProperties = false) => {
  ...
  if (!onlyProperties) {
    data = {
      ...
      _rows: json.features && json.features.length ? json.features.length : 0,
      crs: {
        type: "name",
        properties: {
          name: "EPSG:4326",
...
const autocompleteSpoofer = async (req, res, next) => {
  ...
  if (service == "jordstykker") {
    ...
    var sql = "SELECT result from " + DATAHUB.schema + ".mw_jordstykke_autocomplete";
    ...
    var sql = "SELECT result from " + DATAHUB.schema + ".mw_adresse_autocomplete";
