...
function verifyUser(request) {
  return new Promise(function (resolve, reject) {
    ...
    try {
      yell('Incomming: '+request.body.user.toString())
      yell('Requested: '+request.connection.remoteAddress)
      yell('Requested: '+request.headers['x-forwarded-for'])
    } catch (error) {
      yell(error)
    }
    ...
    if (
      ...
      request.connection.remoteAddress == "::1" || request.connection.remoteAddress == "::ffff:127.0.0.1"
    ) {
