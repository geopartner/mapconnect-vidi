  ...
  if (fileIdent && caseFound) {
    layersToReload.forEach((element) => {
      ...
      if (conf.cosmeticbackgroundlayer) {
        api.turnOn(conf.cosmeticbackgroundlayer);
      }
    });
  }
...
/**
 * Remove cosmetic layers from Vidi.
 */
var removeCosmeticLayers = function() {
  try {
    api.turnOff("vmr.spildevand_wms_niras")
    api.turnOff("vmr.vand_wms_niras")
  } catch (error) {
    //console.log("Error removing cosmetic layers", error)
  } finally {
    return
  }
}
  ...
  metaData.data.forEach(function (d) {
    ...
    try {
      ...
      if (smallKey == layer) {
        ...
        m = d;
        if (confLayer && confLayer.cosmeticbackgroundlayer && !fileIdent) {
          if (smallKey == layer) {
            // Turn on current layers cosmeticbackgroundlayer
            api.turnOn(confLayer.cosmeticbackgroundlayer);
          }
        }
      }
    } catch (error) {
...
*/
    ...
    class DocumentCreate extends React.Component {
      ...
      newButtonClicked() {
        ...
        // reset to default
        $("#" + select_id).val("vmr.vw_spildevand");
        api.turnOn("vmr.spildevand_wms_niras");
      ...
      /**
       * Invoked, when SaveButton is clicked
       */
